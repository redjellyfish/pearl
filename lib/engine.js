...
pearl.tags = {
    ...
    include: function (node, state) {
        ...
        if (node.attribs.import) {
            if (!(node.attribs.import in state.imports)) {
                state.imports[node.attribs.import] = fs.readFileSync(appRoot + '/' + state.config.views + '/' + node.attribs.import + '.' + state.config.ext, "utf8");
            }
            template = htmlparser.parseDOM(state.imports[node.attribs.import]);
            if (template.length != 1) {
                template = { type: "tag", name: "component", children: template, attribs: [] };
            }
        } else {
            //Create a new DOM object from the template object and visit it
            template = htmlparser.parseDOM(state.templates[node.attribs.name])[0];
        }
    ...
    block: function (node, state) {
        ...
        //Store the current block as the parent so it is accessable from the parent element
