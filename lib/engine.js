...
pearl.process = function (dom, model) {
...
pearl.tags = {
    ...
    include: function (node, state) {
        ...
        for (var i = 0; i < overrideBlocks.length; i++) {
            blocks[overrideBlocks[i].attribs.id] = domutils.getOuterHTML(overrideBlocks[i]);
    ...
    resource: function (node, state) {
        ...
        var path = evaluator.rewrite(node.attribs.path, getModel(state)) + "." + type;
    ...
    block: function (node, state) {
        ...
        var block = this.getBlock(node.attribs.id, state);
        if (block === null) {
    ...
    /*,breakpoint: function (node, state) {
        //Debugging only, simply remove the node and break
        domutils.removeElement(node);
        debugger;
    },*/
};
